...
export default function AdminDashboard() {
    ...
    const vacancyQuotas = [
        { department: "Engineering / Dev", filled: 0, target: 15, color: "bg-fuchsia-600" },
        ...
        const fetchAdminMetrics = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    setVacancies([
                        { department: "Engineering / Dev", filled: devs, target: 15, color: "bg-fuchsia-600" },
        ...
        <div className="min-h-screen flex bg-white font-sans">
            ...
            <main className="flex-1 p-4 md:p-8 overflow-y-auto space-y-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6">
                    <div>
                        <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Branch Analytics</h1>
                        <p className="text-sm text-slate-500 mt-1">Audit active data collections, staff distributions, and system runtime history</p>
                    </div>

                    <button
                        onClick={() => navigate('/admin/create-user')}
                        className="px-5 py-3 rounded-xl text-[10px] bg-slate-900 hover:bg-black text-white font-black uppercase tracking-widest transition-all shadow-md shadow-slate-200"
                    >
                        ➕ Onboard New User
                    </button>
                </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            {[
                                ...
                                <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                                    ...
                                    <span className="block text-[10px] text-slate-400 font-bold mt-1 uppercase">{card.context}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
                                <div>
                                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">Department Distribution Chart</h3>
                                    <p className="text-[11px] font-bold text-slate-400 uppercase mt-0.5">Real-time counts parsed from personnel records</p>
                                </div>

                                <div className="space-y-4 pt-2">
                                    {[
                                        { name: 'Engineering / Tech', count: metrics.developerCount, color: 'bg-fuchsia-800' },
                                        ...
                                        { name: 'General Support / Others', count: metrics.otherCount, color: 'bg-slate-400' }
                                    ].map((dept, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className="space-y-1.5">
                                                <div className="flex justify-between text-xs font-bold text-slate-700 uppercase tracking-tight">
                                                    <span>{dept.name}</span>
                                                    <span className="font-mono text-slate-900 font-black">{dept.count} Staff ({Math.round(percentage)}%)</span>
                                                </div>
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
                                <div>
                                    <h3 className="text-xs font-black text-slate-900 uppercase tracking-widest">Hiring Quotas & Vacancies</h3>
                                    <p className="text-[11px] font-bold text-slate-400 uppercase mt-0.5">Track remaining headcounts against corporate metrics</p>
                                </div>
                                        ...
                                        return (
                                            <div key={idx} className="space-y-1.5">
                                                <div className="flex justify-between text-xs font-bold text-slate-700 uppercase tracking-tight">
                                                    ...
                                                    <span className="font-mono font-black text-fuchsia-800">
                                                ...
                                                <div className="flex justify-between text-[9px] font-mono font-black text-slate-400 uppercase">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <div className="lg:col-span-2 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
                                <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Recent Onboarding Operations</h3>
                                {systemLogs.length === 0 ? (
                                    <p className="text-xs text-slate-400 py-4 font-bold uppercase">No deployment entries found inside database records.</p>
                                ) : (
                                        ...
                                        {systemLogs.map((log) => (
                                            <div key={log.id} className="py-3.5 flex justify-between items-center gap-4">
                                                <div className="flex items-center gap-3">
                                                    <span className="text-fuchsia-600 bg-fuchsia-50 px-2 py-0.5 rounded text-[9px] font-black uppercase">SYSTEM_OK</span>
                                                    <span className="text-slate-700 normal-case">{log.text}</span>
                                                </div>
                                                <span className="font-mono text-slate-400 text-[10px] flex-shrink-0">{log.time}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                            ...
                            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between space-y-6">
                                <div>
                                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-4">Branch Framework</h3>
                                    <div className="space-y-3 font-bold text-xs text-slate-600 uppercase">
                                        <div className="flex justify-between border-b border-slate-100 pb-2">
                                            <span className="text-slate-400">Database Context</span>
                                            <span className="text-slate-800 font-mono">MongoDB Atlas</span>
                                        </div>
                                        <div className="flex justify-between border-b border-slate-100 pb-2">
                                            <span className="text-slate-400">Network Engine</span>
                                            <span className="text-slate-800 font-mono">Express REST</span>
                                        </div>
                                        <div className="flex justify-between border-b border-slate-100 pb-2">
                                            <span className="text-slate-400">Token Validation</span>
                                            <span className="text-slate-800 font-mono">HS256 JWT</span>
                                        </div>
                                    </div>
                                </div>

                                <div className="bg-fuchsia-50 border border-fuchsia-100 p-4 rounded-xl text-fuchsia-900">
                                    <span className="block text-[10px] uppercase font-black tracking-widest text-fuchsia-700 mb-1">Security Clearance</span>
                                    <p className="text-[11px] font-medium leading-relaxed normal-case text-fuchsia-600">
