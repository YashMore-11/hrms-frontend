        ...
        <div className="min-h-screen flex bg-white font-sans">
            ...
            <main className="flex-1 p-4 md:p-8 overflow-y-auto space-y-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6">
                    <div>
                        <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Staff Attendance Registry</h1>
                        <p className="text-sm text-slate-500 mt-1">Audit real-time shifts, punch timings, metrics, and workforce check-in status flags</p>
                    </div>

                    <div className="text-right bg-slate-50 border border-slate-200 px-4 py-2.5 rounded-xl font-mono text-xs font-black text-slate-800">
                        {new Date().toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' })}
                    </div>
                </div>

                {/* Real-time Status Filter Tabs */}
                <div className="flex flex-wrap gap-1 bg-slate-50 p-1 rounded-xl text-[10px] font-black uppercase tracking-widest border border-slate-200 w-max">
                    {['all', 'Present', 'Late', 'Leave'].map(category => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg transition-all ${filter === category
                                    ? 'bg-white text-teal-600 shadow-sm font-black border border-slate-200/60'
                            ...
                            <tbody className="divide-y divide-slate-100 font-bold text-slate-700">
                                {filteredRegistry.map((emp) => (
                                    <tr key={emp.empId} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="p-4">
                                            <span className="block font-black text-slate-900 text-sm normal-case">{emp.name}</span>
                                            <span className="block font-mono text-[10px] text-slate-400 mt-0.5">{emp.empId} • {emp.role}</span>
                                        </td>
                                        <td className="p-4 text-slate-500 normal-case">{emp.department}</td>
                                        <td className="p-4 font-mono font-black text-slate-800">{emp.checkIn}</td>
                                        <td className="p-4 font-mono text-slate-400">{emp.checkOut}</td>
                                        <td className="p-4 font-mono font-black text-slate-900">{emp.hours > 0 ? `${emp.hours} hrs` : '--'}</td>
