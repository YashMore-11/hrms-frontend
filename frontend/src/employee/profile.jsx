...
export default function EmployeeProfile() {
  ...
  const [previousRole, setPreviousRole] = useState(""); // ✅ Linked State Node
  ...
  const fetchEmployeeProfileData = async () => {
    ...
    try {
      ...
      setPreviousRole(data.previousRole || ""); // ✅ Loaded cleanly
  ...
  const handleSaveProfile = async (e) => {
    ...
    const updatedPayload = {
      ...
      department,
      previousCompany,
      previousRole, // ✅ Injected safely into request body payload
      yearsOfExperience
    };
  ...
  const handleCancelEdit = () => {
    ...
    if (employee) {
      ...
      setPreviousRole(employee.previousRole || ""); // ✅ Reset cleanly
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              {/* Secure Attached Documents Layout Block (View-Only Compliance Artifacts) */}
              <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm">
                <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-5">Attached Documents Ledger</h3>
                <div className="space-y-3">
                  {[
                    { name: employee.name ? `${employee.name.replace(/\s+/g, '_')}_CV.pdf` : "Curriculum_Vitae.pdf", label: "Professional CV / Resume Document Asset", colorClass: "bg-rose-50 text-rose-600 border-rose-100" },
                    { name: "Government_PAN_Card_Tracer.pdf", label: "PAN Card Compliance Identity File", colorClass: "bg-blue-50 text-blue-600 border-blue-100" },
                    { name: "National_UIDAI_Redacted_Secure.pdf", label: "Identity Verification Registry Data", colorClass: "bg-emerald-50 text-emerald-600 border-emerald-100" }
                  ].map((doc, idx) => (
                    <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-black text-[10px] border ${doc.colorClass}`}>PDF</div>
                        <div>
                          <span className="block text-sm font-bold text-gray-800 tracking-tight">{doc.name}</span>
                          <span className="block text-[9px] font-black text-gray-400 uppercase">{doc.label}</span>
                        </div>
                      </div>
                      <button 
                        type="button" 
                        onClick={() => toast.success(`📑 Streamed document preview for: ${doc.name}`)} 
                        className="text-[10px] font-black text-gray-900 bg-white border border-gray-200 px-5 py-2.5 rounded-lg hover:border-gray-900 transition-all uppercase tracking-widest"
                      >
                        View
                      </button>
                    </div>
