...
const createAuditLog = async (adminId, actionType, targetRole, details) => {
    try {
        await AuditLog.create({ actionBy: adminId, actionType, targetRole, details });
    } catch (err) {
...
router.post('/roles', verifyToken, async (req, res) => {
    try {
        const { roleName, scope, companyId, permissions, subRoleCategory, timeBasedAccess } = req.body;

        const newRole = new Role({
            roleName, scope, companyId, permissions, subRoleCategory, timeBasedAccess
        });
...
router.post('/roles/:id/clone', verifyToken, async (req, res) => {
    try {
        ...
        const { targetCompanyId, newRoleName } = req.body; 
        ...
        res.status(201).json({ message: "Role cloned successfully", role: clonedRole });
    } catch (err) {
        res.status(500).json({ message: "Failed to clone role", error: err.message });
    }
});
